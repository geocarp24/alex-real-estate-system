...
### 🚪 ORÁCULO GATE — bloqueante antes de generar (2026-04-23, NO NEGOCIABLE)
Orden directa de Jorge. El Creativo NO invoca tools de generación (`banana-claude`, `claude-video-*`, HeyGen, Replicate, Veo, Runway, Fooocus, `remotion-ads`, Google AI Studio API) sin aprobación del Oráculo. Workflow: idea → Creativo propone spec JSON → 🚪 Oráculo evalúa 5 ejes (audience_fit, brand_voice, legal_compliance, negative_reactions, roi_potential) via MiroFish simulación de personas WI → APROBADO / MODIFICAR (hasta 3 iter) / RECHAZADO → solo si APROBADO el Creativo ejecuta tools → Programador publica. Veto automático: `legal_compliance < 7` = RECHAZADO sin override. Spec v1.0 del Oráculo en `agents/oraculo.md`. Creativo actualizado a v6 con gate integrado. Regla en `CLAUDE.md` sección "🚪 ORÁCULO GATE".
