...
async function gather(cfg, weekStr) {
  ...
  const [marketing, seo, ads, compliance, espia, leadScores, leads, deals, content, emails, emailEvents] =
    await Promise.all([
      ...
      airtableFetch(cfg, "leads_table_id", "maxRecords=100&sort[0][field]=Created&sort[0][direction]=desc").catch(() => ({ records: [] })),
      airtableFetch(cfg, "deals_table_id", "maxRecords=100&sort[0][field]=Created&sort[0][direction]=desc").catch(() => ({ records: [] })),
      airtableFetch(cfg, "content_queue_table_id", "maxRecords=50&sort[0][field]=Created&sort[0][direction]=desc").catch(() => ({ records: [] })),
