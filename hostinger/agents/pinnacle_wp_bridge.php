 ...
 * Supports WP 6.8+ $generic$ fast-hash format plus legacy bcrypt and phpass.
 */
...
// Supports three hash formats that WordPress has used for application
// passwords across versions:
//   $generic$... → wp_fast_hash (WP 6.8+), verified with wp_verify_fast_hash
//   $2y$...      → bcrypt, verified with password_verify
//   $P$...       → phpass portable hash, verified with PasswordHash
    ...
    if ($user instanceof WP_User && user_can($user, 'manage_options') && class_exists('WP_Application_Passwords')) {
        ...
        if (is_array($hashed_passwords)) {
            foreach ($hashed_passwords as $item) {
                $stored = (string) $item['password'];
                $matched = false;

                if (str_starts_with($stored, '$generic$') && function_exists('wp_verify_fast_hash')) {
                    $matched = wp_verify_fast_hash($cleaned_pass, $stored);
                } elseif (str_starts_with($stored, '$2')) {
                    $matched = password_verify($cleaned_pass, $stored);
                } elseif (str_starts_with($stored, '$P$') || str_starts_with($stored, '$H$')) {
