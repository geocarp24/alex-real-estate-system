...
switch ($action) {
    ...
    case 'start_lead': {
        // Anti-bot: honeypot must be empty, elapsed_ms must be >= 5000
        ...
        if ($place_id === '') {
            // Extra protection: if no place_id came back, reject (forces Google autocomplete selection)
            $log('warn', 'no place_id — possible manual address typed');
            // Don't reject outright; accept but flag it
        }
        ...
        // Generate SMS code + session token
        ...
        set_transient(pp_lead_session_key($lead_id), $session, 1800); // 30 min
        ...
        if (!$sent['ok']) {
            ...
            // We still created the lead — return without code success; user may retry
    ...
    case 'verify_phone': {
        ...
        pp_airtable_update($lead_id, ['Stage' => 'New Lead']); // Verified — we can mark differently later
    ...
    case 'update_lead': {
        ...
        // Airtable update with enriched fields
        $update = [
